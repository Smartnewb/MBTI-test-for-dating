...
export const analyzeResponseConsistency = (questions, responses) => {
  ...
  Object.keys(dimensionQuestions).forEach(dimension => {
    ...
    // 일관성 점수 계산 (0-100)
    const dimensionConsistency = totalPairs > 0 
  ...
  return dimensionCount > 0 
    ? Math.round(totalConsistencyScore / dimensionCount) 
    : 0;
};
 ...
 */
export const calculateConfidenceScore = (questions, responses, completeness, dimensionCertainty) => {
  // 응답 일관성 분석
